...
sr() {
    sudo systemctl restart "$@" || journalctl "${@/#/-u}" -S "5min ago" --no-pager
}
ss() {
...
ca() {
    local target=${@:2}
    eval "_comp_lazy_$1() {
        unset -f _lazy_comp_$1
        _comp_load $2

        local compopts=\$(complete -p $2 | rg -oP '^.*(?= -F)')
        local compfunc=\$(complete -p $2 | rg -oP -- '-F \K[^ ]*')

        if [[ $# -gt 2 ]]
        then
            eval \"_comp_wrap_$1() {
                COMP_LINE=\\\${COMP_LINE/$1/$target}
                COMP_POINT=\\\$((COMP_POINT + ${#target} - ${#1}))
                COMP_WORDS=($target \\\"\\\${COMP_WORDS[@]:1}\\\")
                COMP_CWORD=\\\$((COMP_CWORD + $# - 2))
                \$compfunc \\\"\\\$@\\\"
            }\"
            compfunc=_comp_wrap_$1
        fi

        \$compopts -F \$compfunc $1
        \$compfunc  \"\$@\"
    }"
    complete -F _comp_lazy_$1 $1
}
...
ca rc rsync
ca rv rsync
ca sr systemctl restart
ca ss systemctl status
