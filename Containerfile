...
RUN curl -sL https://github.com/samtools/samtools/releases/latest/download/samtools-1.23.1.tar.bz2 | tar xj && \
    cd samtools-* && ./configure --prefix=/usr && make -j && make install && cd .. && rm -r samtools-*      && \
    curl -sL https://github.com/samtools/bcftools/releases/latest/download/bcftools-1.23.1.tar.bz2 | tar xj && \
    cd bcftools-* && ./configure --prefix=/usr && make -j && make install && cd .. && rm -r bcftools-*      && \
    curl -sL https://github.com/samtools/htslib/releases/latest/download/htslib-1.23.1.tar.bz2     | tar xj && \
