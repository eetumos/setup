...
RUN --mount=type=cache,dst=/var/cache/libdnf5 --mount=type=bind,src=build-env/dnf.conf,dst=/etc/dnf/dnf.conf,z \
    dnf install -y tmux nnn neovim ripgrep fzf pwgen                                                           \
                   {h,b}top {s,l}trace iotop-c nethogs nmap iperf3 socat wev                                   \
...
RUN curl -sL https://github.com/samtools/samtools/releases/latest/download/samtools-1.24.tar.bz2 | tar xj && \
    cd samtools-* && ./configure --prefix=/usr && make -j && make install && cd .. && rm -r samtools-*    && \
    curl -sL https://github.com/samtools/bcftools/releases/latest/download/bcftools-1.24.tar.bz2 | tar xj && \
...
RUN curl -sLOOO -o date-menu-formatter@marcinjakubowski.github.com.strip.zip -o lan-ip-address@mrhuber.com.strip.zip                                  \
        ...
        https://github.com/ubuntu/Tiling-Assistant/releases/latest/download/tiling-assistant@leleat-on-github.shell-extension.zip                     \
