...
RUN --mount=type=cache,dst=/var/cache/libdnf5 --mount=type=bind,src=build-env/dnf.conf,dst=/etc/dnf/dnf.conf,z            \
    ...
    dnf install -y langpacks-fi gnome-tweaks cockpit{,-machines} libvirt-daemon  \
                   ...
                   unrar p7zip-plugins bsdtar icoutils tesseract                 \
                   ffmpeg compat-ffmpeg4 mediainfo mkvtoolnix                    \
...
RUN dnf install -y {openxr,vulkan-loader,wayland,wayland-protocols,systemd,libdrm,hidapi,libusb1,libv4l,eigen3}-devel glslang && \
